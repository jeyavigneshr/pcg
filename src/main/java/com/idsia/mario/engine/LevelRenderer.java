...
import com.idsia.mario.engine.level.*;
import com.viewer.MarioLevelViewer;
...
public class LevelRenderer {
	...
	/**
	 * Determine the default graphics configuration for the current system. Used to
	 * preview Mario levels.
	 * 
	 * @return
	 */
	 ...
	 * about the area to view. The level is then drawn to the Graphics2D, which is
	...
	public void render(Graphics g, int tick, float alpha) {
		...
		for (int x = xCam / 16; x <= (xCam + width) / 16; x++)
			for (int y = yCam / 16; y <= (yCam + height) / 16; y++) {
				...
				/*
				 * else if (b == Level.TILE_BONUS) { int animTime = (tick / 3) % 4; int yo = 0;
				 * if (x >= 0 && y >= 0 && x < level.width && y < level.height) yo =
				 * level.data[x][y]; if (yo > 0) yo = (int) (Math.sin((yo - alpha) / 4.0f *
				 * Math.PI) * 8); g.drawImage(Art.mapSprites[(4 + animTime)][0], (x << 4) -
