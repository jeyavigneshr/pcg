...
import com.idsia.mario.engine.GlobalOptions;
...
public class JsonReader {
	...
	/**
	 * Creates integer list representation of multiple levels from a json file whose
	 * path is designated by the parameter.
	 * 
	...
	/**
	 * Jacob: Designed entry point into method that does not require a file, only
	...
	// The method above may unnecessarily duplicate some functionality of this
	// method
	public static List<List<List<Integer>>> JsonToInt(String myJSONString) {
		...
		JsonArray jarray1 = (JsonArray) jsonParser.parse(myJSONString);
		List<List<List<Integer>>> myReturnList = new ArrayList<List<List<Integer>>>();
...
//		}
//		return myArray;
		String clean = myJSONString.substring(1, myJSONString.length()-1);
		String[] cleanArray = clean.split(",");
		double[] myArray = Arrays.stream(cleanArray).mapToDouble(Double::parseDouble).toArray();
		return myArray;
	}
}
