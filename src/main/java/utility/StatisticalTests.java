...
public class StatisticalTests {
	...
	/**
	 * Calculates the probability with which the null hypothesis (that the means are
	 * equal) can be rejected in favour of the alternative hypothesis that they are
	 * not equal (hence uses a two-sided test). The samples are not paired and do
	...
	/**
	 * Calculates the probability with which the null hypothesis (that the means are
	 * equal) can be rejected in favour of the alternative hypothesis that one is
	 * greater than the other (hence uses a single-sided test). The samples are not
	...
	/**
	 * Calculates the probability with which the null hypothesis (that the means are
	 * equal) can be rejected in favour of the alternative hypothesis. Uses a
	...
	/**
	 * Applies a one-sided t-test to two arrays of paired samples. The arrays must
	 * be the same size; failure to ensure this could cause an
	...
	/**
	 * Applies a two-sided t-test to two arrays of paired samples. The arrays must
	 * be the same size; failure to ensure this could cause an
	 * ArrayOutOfBoundsException.
	 */
	...
	/**
	 * Applies a t-test to two arrays of paired samples. One or two-sided is chosen
	 * depending on the value of the boolean variable 'two-sided'. The arrays must
	 * be the same size; failure to ensure this could cause an
	 * ArrayOutOfBoundsException.
	 */
	 ...
	 * s1.length - 1; double m1 = Stats.mean(s1); double m2 = Stats.mean(s2); double
	 * stderr = Math.sqrt((var1 + var2 - 2 * covar) / s1.length); double t = (m1 -
	 * m2) / stderr; return tTest(t, nu); }
	 */
	/**
	 * Applies the t-test to an array of the differences between paired samples of
	 * observations.
	 */
	...
	/**
	 * This method returns the distance x from the mean m such that the area under
	 * the t distribution (estimated from the data d) between (m - x) and (m + x) is
	 * equal to conf.
	 * <p/>
	 * In other words, it finds the desired confidence interval of the mean of the
	 * population from which the data is drawn. For example, if conf = 0.95, then
	 ...
	 * @return the spread around the sample mean of the population mean within that
	 *         confidence interval
	 */
	...
	/**
	 * Bin root - a binary search for a square root. I wrote this purely to check my
	 * recollection of how this kind of search can be used to invert functions.
	 */
	...
	/**
	 * Applies the two-sided t-test given the value of t and nu. To do this it calls
	 * betai.
	 */
	...
	/**
	 * Applies the single-sided t-test given the value of t and nu. To do this it
	 * calls betai.
	 */
	...
	protected static double gammln(double xx) {
		...
		ser = 1.0 + 76.18009173 / (x + 1.0) - 86.50532033 / (x + 2.0) + 24.01409822 / (x + 3.0)
				- 1.231739516 / (x + 4.0) + 0.120858003 / (x + 5.0) - 0.536382e-5 / (x + 6.0);
	 ...
	 * 294) to check the calculation of the confidence intervals for the mean of a
	 * dataset.
	 * <p/>
	 ...
	 * <p/>
	 * <p/>
	 * This matches closely with the book - any differences are due to small errors
	 * in the book version due to the limited number of decimal places used (2) in
