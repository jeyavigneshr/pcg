...
public class Mario extends Sprite implements Cloneable {
	...
	public void move() {
		...
		if (sliding) {
			for (int i = 0; i < 1; i++) {
				world.addSprite(new Sparkle(this.world, (int) (x + Math.random() * 4 - 2) + facing * 8,
	...
	private void calcPic() {
		...
		if (onGround && ((facing == -1 && xa > 0) || (facing == 1 && xa < 0))) {
			...
			if (xa > 3 || xa < -3) {
				for (int i = 0; i < 3; i++) {
					world.addSprite(
							new Sparkle(this.world, (int) (x + Math.random() * 8 - 4), (int) (y + Math.random() * 4),
			...
			if (this.doBlocks) {
				...
				for (int xx = 0; xx < 2; xx++)
					for (int yy = 0; yy < 2; yy++)
						world.addSprite(new Sparkle(this.world, x * 16 + xx * 8 + (int) (Math.random() * 8),
