...
public class AStarSimulator {
	...
	private void search(int repetitions, long startTime) {
		...
		while (posPool.size() != 0 && ((levelScene.mario.x - currentSearchStartingMarioXPos < 176) || !currentGood)
	...
	private SearchPos pickBestPos(ArrayList<SearchPos> posPool) {
		...
		for (SearchPos current : posPool) {
			...
			float currentCost = current.getRemainingTime() + current.timeElapsed * 0.79f + jumpModifier; // slightly
