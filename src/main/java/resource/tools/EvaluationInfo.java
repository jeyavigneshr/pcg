...
public class EvaluationInfo {
	...
	public double[] toDouble() {
		return new double[] { marioStatus, lengthOfLevelPassedPhys, totalLengthOfLevelCells, timeSpentOnLevel,
				numberOfGainedCoins, totalActionsPerfomed, totalFramesPerfomed, computeBasicFitness() };
	}
	...
	public String toString() {
		...
		ret += "\n                     Passed (Cells) : "
				+ df.format((double) lengthOfLevelPassedCells / totalLengthOfLevelCells * 100) + "% ( "
				+ lengthOfLevelPassedCells + " of " + totalLengthOfLevelCells + ")";
		ret += "\n             Time Spent(Fractioned) : " + timeSpentOnLevel + " ( "
				+ df.format((double) timeSpentOnLevel / totalTimeGiven * 100) + "% )";
		ret += "\n              Time Left(Fractioned) : " + timeLeft + " ( "
				+ df.format((double) timeLeft / totalTimeGiven * 100) + "% )";
