...
public class ToolsConfigurator extends JFrame {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -952034202370145269L;
...
//        marioComponentFrame.setTitle(evaluationOptions.getAgent().getName() + " - Mario Intelligent 2.0");
	...
	public TextArea TextAreaConsole = new TextArea("Console:"/* , 8,40 */); // Verbose all, keys, events, actions,
																			// observations
	...
	private BasicArrowButton upFPS = new BasicArrowButton(BasicArrowButton.NORTH),
			downFPS = new BasicArrowButton(BasicArrowButton.SOUTH);

	// TODO allowed time to use.
	// TODO : change agent on the fly. Artificial Contender concept? Human shows how
	// to complete this level? Fir 13:38.
...
//            }
		...
		public void itemStateChanged(ItemEvent ie) {
			...
			if (ob == CheckboxShowGameViewer) {
				...
				LOGGER.println(
						"Stop simulation if Win Criteria Turned " + (GlobalOptions.StopSimulationIfWin ? "on" : "off"),
						LOGGER.VERBOSE_MODE.INFO);
		...
		public void stateChanged(ChangeEvent changeEvent) {
			...
			if (ob == JSpinnerLevelRandomizationSeed) {
				...
				if (Integer
						.parseInt(JSpinnerLevelLength.getValue().toString()) < LevelGenerator.LevelLengthMinThreshold)
